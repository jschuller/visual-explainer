...
**Optional hero image** — check `which gemini-image 2>/dev/null || which surf 2>/dev/null`. If available, consider generating a conceptual illustration via `gemini-image "prompt" --generate-image /tmp/file.png --aspect-ratio 16:9` that captures the feature's essence. Use for abstract concepts that benefit from visual metaphor. Skip for purely structural changes. Embed as base64 data URI using the `.hero-img-wrap` pattern from css-patterns.md.
